...
//   ADIAuth.reachable(timeoutMs) → Promise<boolean>. False if the hosted
//                                  backend is unreachable (→ fall back to BYOK).
...
(function () {
  ...
  // P0-A: lightweight reachability probe. Resolves true if the hosted backend
  // answers with ANY HTTP response (host resolves + server up), false on a DNS
  // failure / network error / timeout. Used by adiInitAuth so a paused
  // free-tier Supabase falls back to BYOK instead of bricking the app.
  // NOTE: uses Promise.race + setTimeout, never AbortController (rule #1 — it
  // breaks inside the Claude.ai iframe). A slow request is simply ignored.
  async function reachable(timeoutMs = 4000) {
    if (!isConfigured()) return false;
    const base = String(CFG.supabaseUrl).replace(/\/+$/, '');
    try {
      const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), timeoutMs));
      const probe = fetch(base + '/auth/v1/health', {
        headers: { apikey: CFG.supabaseAnonKey },
        cache: 'no-store',
      });
      const r = await Promise.race([probe, timeout]);
      return !!r; // any completed response means the project is awake
    } catch (_) {
      return false; // NXDOMAIN / offline / timeout
    }
  }
  ...
  async function usage() {
    ...
    try {
      ...
      // Display-only quota (the authoritative gate is public.plan_quota() server-side, P1-A).
      // Keep these numbers in sync with that function. (`quotaNum` was a dead config lookup.)
      const quota = ({ trial: 1, pro: 50, team: 250 }[plan] || 0);
  ...
  async function signUp(email, password, displayName) {
    const c = client();
    if (!c) return { user: null, session: null, error: { message: 'Auth not configured' } };
    // P0-B: supabase-js throws (not returns) on a network failure; catch it so
    // callers always get a structured {error} instead of an unhandled rejection.
    try {
      ...
      return { user: null, session: null, error: { message: 'Network error — could not reach the sign-up service.', isNetwork: true, cause: e?.message } };
    }
  }
  ...
  async function signIn(email, password) {
    const c = client();
    if (!c) return { user: null, session: null, error: { message: 'Auth not configured' } };
    // P0-B: catch thrown network errors (see signUp note).
    try {
      const { data, error } = await c.auth.signInWithPassword({ email, password });
      return { user: data?.user || null, session: data?.session || null, error };
    } catch (e) {
      return { user: null, session: null, error: { message: 'Network error — could not reach the sign-in service.', isNetwork: true, cause: e?.message } };
    }
  }
